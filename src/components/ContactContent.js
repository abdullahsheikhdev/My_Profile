...
export default function ContactContent() {
  ...
  return (
    <div className="flex-1 max-w-full text-gray-400 p-6 sm:p-10 flex flex-col min-h-full pb-10">
      ...
      <div className={`border-t ${darkMode ? "gradient-border-content" : "gradient-border-content-white"} w-full pt-8 flex flex-col gap-10`}>
        ...
        <div className="grid mt-2 grid-cols-1 xl:grid-cols-2 gap-y-6 gap-x-8 w-full text-sm font-mono tracking-wide text-gray-300">
          <div className="flex items-center justify-between w-full">
            ...
            <span className="text-[#a4a4a4] whitespace-nowrap text-right">Bangladesh, Dhaka</span>
          </div>

          <div className="flex items-center justify-between w-full">
            <span className="whitespace-nowrap">EMAIL:</span>
            <div className="flex-1 mx-2 sm:mx-4 border-b-2 border-dotted border-gray-600 mt-1 min-w-4"></div>
            <span className="text-[#a4a4a4] whitespace-nowrap text-right truncate max-w-37.5 sm:max-w-none">rk.abdullahh.dev@gmail.com</span>
          </div>

          <div className="flex items-center justify-between w-full">
            <span className="whitespace-nowrap">PHONE:</span>
            <div className="flex-1 mx-2 sm:mx-4 border-b-2 border-dotted border-gray-600 mt-1 min-w-4"></div>
            <span className="text-[#a4a4a4] whitespace-nowrap text-right">+880 1843 904218</span>
          </div>

          <div className="flex items-center justify-between w-full">
            <span className="whitespace-nowrap">FREELANCE:</span>
            <div className="flex-1 mx-2 sm:mx-4 border-b-2 border-dotted border-gray-600 mt-1 min-w-4"></div>
            <span className="text-[#a4a4a4] whitespace-nowrap text-right">Available</span>
          </div>
        </div>
      </div>

      {/* Contact Form Section Title */}
      <h2 className={`text-xl font-bold ${darkMode ? "text-white" : "text-black"} relative inline-block mb-6 mt-16`}>
        <span className="absolute -left-2 top-1.5 w-8 h-8 bg-linear-to-br from-[#F6B846] to-[#373126] rounded-full opacity-40 -z-10"></span>
        <span className="relative z-10 text-[#F6B846]">C</span>ontact Form
      </h2>

      {/* Contact Form Container with top border */}
      <div className={`border-t ${darkMode ? "gradient-border-content" : "gradient-border-content-white"} w-full pt-8 flex flex-col gap-8`}>
        ...
        <div className="mt-2 text-left mb-4">
          <button className={`flex items-center text-[13px] font-bold transition-all uppercase tracking-widest hover:text-[#F6B846] ${darkMode ? 'text-white' : 'text-black'}`}>
            SEND MESSAGE 
